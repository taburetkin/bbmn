...
var RoutesMixin = {
	...
	_buildRoutesContexts: function _buildRoutesContexts() {
		...
		var result = _.reduce(routes, function (memo, route, index$$1) {
			var context = _this._normalizeRoutesContextRoute(route, index$$1, config);
			context = _this.fixRouteContext(context);
			if (_.isObject(context)) {
				memo.push(context);
			}
			return memo;
	...
	fixRouteContext: function fixRouteContext(context) {
		return context;
	},
	_normalizeRoutesContextRoute: function _normalizeRoutesContextRoute(arg, index$$1) {
		...
		context.getUrl = function () {
			...
			return this.routeForGetUrl.replace(/:([^/?]+)/, function (found, group) {
				return data[group];
			});
		};
